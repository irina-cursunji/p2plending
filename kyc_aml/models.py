...
class KycApplication(BaseModel):
    ...
    identification_type = models.CharField(
        max_length=21,
        choices=ModelChoices.PHOTO_INDENTIFICATION_TYPE,
        default=ModelChoices.PASSPORT,
...
class KycSetting(SingletonModel):
    """Model represents KYC Settings and Configuration"""
    ...
    enable_kyc = models.BooleanField(
        ...
        help_text=_("Set to True to activate KYC Application on system. Default sets to `False`."))
 
    allow_resubmission = models.BooleanField(
        default=False,
        verbose_name=_('Allow Re-submission'),
        help_text=_("""Toggle to turn On/Off to allow client to re-submit Application if KYC attempt fails after the specific number of tries."""))
 
    number_kyc_tries = models.PositiveIntegerField(
        default=1,
        verbose_name=_('Allowed KYC tries'),
        help_text=_('The number of times a client is allowed to attempt KYC Application.'))
 
    run_check_on_expiry = models.BooleanField(
        default=False,
        verbose_name=_('Request Verification on Expiry'),
        help_text=_("Toggle to require users to go through KYC Verification on expiration of their ID."))
 
    days_to_send_notification = models.PositiveIntegerField(
        default=30,
        verbose_name=_('Verification Notification - (In Days)'),
        help_text=_('If `run_check_on_expiry` is turned on, indicate the number of days in which client receives a reminder of ID Expiration and request for re-verification.'))
 
    review_frequenxy = models.PositiveIntegerField(
        default=30,
        verbose_name=_('Review Frequency - (In Days)'),
        help_text=_('The frequency based on which customer must be reviewed.'))
 
    kyc_system_type = models.CharField(
        choices=KYC_SYSTEM_TYPE,
        max_length=9,
        verbose_name=_('KYC System'),
        blank=True, null=True)
    # endregion
